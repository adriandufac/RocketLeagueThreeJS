...
export default class Walls {
  constructor() {
    ...
    this.arenaHeight = sizes.height;

    this.wallThickness = 0.5;
    this.wallColor = 0x808080;
    this.wallOpacity = 0.8;
  ...
  createWalls() {
    ...
    this.createSideWall(true); // +X side (right wall)
    this.createSideWall(false); // -X side (left wall)
    this.createEndWall(true); // +Z side (far wall with goal)
    this.createEndWall(false); // -Z side (near wall with goal)
  }

  createSideWall(isPositiveX) {
  ...
  createEndWall(isPositiveZ) {
    ...
    // Goal dimensions
    const goalWidth = sizes.goalWidth;
    const goalHeight = sizes.goalHeight;
    ...
    if (this.physics) {
      [topWall, leftWall, rightWall].forEach((wallPart) => {
        this.physics.addEntity(wallPart);
      });
    }
